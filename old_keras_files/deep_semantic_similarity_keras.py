...
# The first dimension is None because the queries and documents can vary in length.
    ...
    doc = Input(shape = (None, WORD_DEPTH), name="document")
...
# a single 1 x 1 kernel.
    ...
    with_gamma = Convolution1D(1, 1, padding = "same", input_shape = (1, 1), activation = "linear", use_bias = False, weights = [weight])(cosine_3d) # See equation (5).
    with_gamma = Reshape((1, ))(with_gamma)

# Finally, we use the softmax function to calculate P(D+|Q).
    prob = Activation("softmax")(with_gamma) # See equation (5).

# We now have everything we need to define our model.
    inputs = {"claim":query, "evidence": doc}
    model = Model(inputs = [query, doc], outputs = prob)
